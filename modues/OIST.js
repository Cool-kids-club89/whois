...
export async function showUserProfile(username, container){
  ...
  try{
    ...
    extractKeywords(profile.bio||"", username, dynamicKeywords);

    const reposRes = await fetch(`https://api.github.com/users/${username}/repos`);
    const repos = await reposRes.json();
    repos.forEach(r=>{extractKeywords(r.name, username, dynamicKeywords); extractKeywords(r.description, username, dynamicKeywords)});

    const orgsRes = await fetch(`https://api.github.com/users/${username}/orgs`);
    const orgs = await orgsRes.json();
    orgs.forEach(o=>extractKeywords(o.login, username, dynamicKeywords));

    // Merge with local profile
    const localContainer = document.getElementById("localProfile");
    let localKeywords = {};
    if(localContainer?.innerText?.trim()) extractKeywords(localContainer.innerText, username, localKeywords);

    const mergedKeywords = {...localKeywords,...dynamicKeywords};

    detectAliases(username);
    buildFingerprint(username, mergedKeywords);
    inferPersona(mergedKeywords);
    clusterUsers();
    buildGraph(username);
    ...
    displayClusters();
    renderGraph();
    displayKeywordConfidence(mergedKeywords);
    scoreSource("GitHub", mergedKeywords);
